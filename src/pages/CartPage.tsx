...
const CartPage = () => {
  ...
  return (
    <div className="min-h-screen max-w-3xl mx-auto px-4 py-6">
              ...
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  {item.selectedPriceLabel && (
                    <span className="text-xs text-muted-foreground">({item.selectedPriceLabel})</span>
                  )}
                </div>
              ...
              {item.selectedAddOns.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                  {item.selectedAddOns.map((ao) => (
                    <span key={ao.id} className="text-xs px-2 py-1 rounded-full bg-secondary text-secondary-foreground">
                      {ao.name} (+₹{ao.price})
                    </span>
                  ))}
                </div>
      ...
      <Link
        to="/checkout"
        className="block w-full text-center py-4 rounded-2xl waffle-gradient text-primary-foreground font-semibold text-lg hover:opacity-95 transition-opacity"
      >
        Proceed to Checkout
      </Link>
    </div>
